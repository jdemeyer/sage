...
class Maxima(MaximaAbstract, Expect):
    ...
    def _synchronize(self):
        ...
        try:
            ...
            # Send only one interrupt. On OS X 10.4, maxima takes a
            # long time to process one interrupt (7.5 seconds on an
            # idle system, but up to a minute on a loaded system)
            # and gets confused by multiple interrupts.  Instead,
            # send just one interrupt and wait.
            # See Trac #9361.
            self.interrupt(tries=1, timeout=120)
