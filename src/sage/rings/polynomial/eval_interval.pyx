...
def eval_polynomial_interval(pol, v, long recurse=0):
    """
    ...
    return eval_polynomial_interval_recursive(coeffs, v, recurse)


cpdef eval_polynomial_interval_recursive(list coeffs, v, long recurse):
    if len(coeffs) == 2:  # degree 1
        return (v * coeffs[1]) + coeffs[0]

    if recurse == 0:
        return eval_polynomial_interval_0(coeffs, v)

    recurse -= 1

    # Compute f(x) as
    # f(x) = f(a) + (x - a) f'(a) + (x - a)[f'(c) - f'(a)]
    # where c is an unknown element in the interval [a,x].
    raise NotImplementedError


cpdef eval_polynomial_interval_0(list coeffs, v):
    cdef Py_ssize_t deg = len(coeffs) - 1
...
cpdef eval_polynomial_error(list coeffs, x, e):
    ...
    A = logA.exp()
    B = logB.exp()
    return B * geom_sum_diff(A*xmag, A*emag, deg+1)


def geom_sum_diff(RealNumber x, RealNumber e, unsigned long n):
    r"""
    Define `(q)[n] = 1 + q + q^2 + ... + q^{n-1}`. Return an
    approximation of `(x+e)[n] - (x)[n]` assuming `x \geq 0` and `e \geq 0`.
    ...
    All calculations are done to a sufficiently high precision to avoid
    ...
    TESTS::
        ...
        ....:
        ....:     # Adjust x and e to maximize bad cases (e.g. close to 1)
        ....:     k = randrange(10)
        ....:     if k == 0:
        ....:         x = 0
        ....:     elif k == 1:
    ...
    # Given that x >= 0 and e > 0, the answer must be positive.
    # We compute the rounding mode and its opposite.
    if R.rnd == MPFR_RNDN:
        rnd = MPFR_RNDN
        rndopp = MPFR_RNDN
