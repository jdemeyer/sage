...
class NumberFieldComplexEmbedding(RingHomomorphism_im_gens):
    ...
    def __init__(self, parent, im_gens, check=None):
        ...
        If we give a completely bogus root, we get an error::
            ...
            Traceback (most recent call last):
            ...
            ArithmeticError: cannot refine embedding (precision too low?)
        """
        ...
        from sage.rings.all import AA, QQbar, PolynomialRing
        from sage.rings.polynomial.complex_roots import refine_root

        # refine_root() is a complex algorithm, so we really need CIF
        # here, even in the real case
        C = parent.codomain()
        IF = ComplexIntervalField(C.precision())
        IFx = PolynomialRing(IF, 'x')

        pol = parent.domain().polynomial()
        r = refine_root(IFx(pol), IFx(pol.derivative()), IF(g0), IF)
        if r is None:
            raise ArithmeticError("cannot refine embedding (precision too low?)")
        if not g0.imag():  # Given root was real
            ga = AA.polynomial_root(pol, r.real())
        else:
            ga = QQbar.polynomial_root(pol, r)
        ...
        # not an exact root.
        super(NumberFieldComplexEmbedding, self).__init__(parent, (C(ga),), check=False)
